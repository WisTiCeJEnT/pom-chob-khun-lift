...
def get_user_activity_list(show):
    if(connection):
        cursor = connection.cursor()
        query_string = f"""
            ...
            WHERE user_data.user_id = user_activity.user_id
            AND NOT user_data.is_deleted
            ORDER BY user_activity.id DESC
            limit %s;
            """
        cursor.execute(query_string, (show, ))
        query_result = cursor.fetchall()
        cursor.close()
        return query_result
    return 'database error'
...
def get_user_data(user_id):
    if(connection):
        ...
        query_string = f"""
            ...
            FROM user_data, user_permission
            WHERE user_data.user_id = user_permission.user_id
            AND user_data.user_id = %s
            AND NOT user_data.is_deleted;
            """
        ...
        query_result[-1] = ([0,0,0,0]+[int(floor) for floor in bin(query_result[-1])[2:]])[-4:]
        cursor.close()
        return query_result
    return 'database error'
